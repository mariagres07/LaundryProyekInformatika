    ...
    <div class="container p-4">
        ...
        <div class="tab-content">
            ...
            <div id="tabCategories" class="tab-pane fade show active">
                <div id="kategoriList">
                    ...
                    <div class="service-item">
                        <form action="{{ route('kategori.update', $c->idKategoriItem) }}" method="POST"
                            class="d-flex align-items-center flex-grow-1 form-update">
                            @csrf
                            @method('PUT')
                            <div class="service-left">
                                <img src="/{{ $c->icon ?? 'default.png' }}" alt="{{ $c->namaKategori }}">
                                <input name="namaKategori" type="text" class="name-input form-control border-0"
                                    value="{{ $c->namaKategori }}" required>
                            </div>
                            <input name="hargaPerItem" type="number" class="price-input form-control"
                                value="{{ $c->hargaPerItem }}" required>
                            <button type="submit" class="update-btn">Simpan</button>
                        </form>

                        <form action="{{ route('kategori.destroy', $c->idKategoriItem) }}" method="POST"
                            class="form-delete">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="delete-btn" title="Hapus kategori"><i
                ...
                <form id="formKategori" class="service-item d-none mt-3" method="POST"
                    action="{{ route('kategori.store') }}">
                    @csrf
                    <div class="service-left">
                        <img src="/default.png" alt="Tambah">
                        <input name="namaKategori" class="form-control border-0" placeholder="Masukkan nama kategori"
                            required>
                    </div>
                    <input name="hargaPerItem" type="number" min="0" class="form-control price-input"
            ...
            <div id="tabPakets" class="tab-pane fade">
                @forelse($pakets as $p)
                <div class="service-item">
                    <form action="{{ route('layanan.update', $p->idLayanan) }}" method="POST"
                        class="d-flex align-items-center flex-grow-1 form-update">
                        @csrf
                        @method('PUT')
                        <div class="service-left">
                            <img src="/{{ $p->icon ?? 'default.png' }}" alt="{{ $p->namaLayanan }}">
                            <input name="namaLayanan" type="text" class="name-input form-control border-0"
                                value="{{ $p->namaLayanan }}" required>
                        </div>
                        <input name="hargaPerKg" type="number" min="0" class="price-input form-control"
                            value="{{ $p->hargaPerKg }}" required>
                        <button type="submit" class="update-btn">Simpan</button>
                    </form>

                    <form action="{{ route('layanan.destroy', $p->idLayanan) }}" method="POST"
                        class="form-delete d-flex align-items-center">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="delete-btn" title="Hapus paket"><i
                                class="bi bi-trash-fill"></i></button>
                    </form>
                </div>
                @empty
                ...
                <!-- Form tambah paket -->
                <div class="ms-2">
                    <form id="formPaket" class="service-item d-none mt-3" action="{{ route('layanan.store') }}"
                        method="POST">
                        @csrf

                        <div class="service-left">
                            <img src="/default.png" alt="Tambah">
                            <input name="pewangi" class="form-control border-0"
                                placeholder="Nama pewangi (Vanilla, Coffee, dll)" required>
                        </div>

                        <select name="jenisPaket" class="form-control price-input" required>
                            <option value="">Pilih Paket</option>
                            <option value="reguler">Reguler</option>
                            <option value="express">Express</option>
                        </select>

                        <input name="hargaPerKg" type="number" min="0" class="form-control price-input"
                            placeholder="Harga per Kg" required>
