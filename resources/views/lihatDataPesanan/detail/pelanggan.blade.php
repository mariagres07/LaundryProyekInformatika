        ...
        <div class="info-card">
            ...
            <div class="row mb-2">
                <div class="col-sm-7 data-value">
                    ...
                    $items = [
                    'Pakaian' => $pesanan->pakaian ?? 0,
                    'Sprei' => $pesanan->seprai ?? 0,
                    'Handuk' => $pesanan->handuk ?? 0,
                    ];

                    // Filter hanya yang jumlahnya > 0
                    $filtered = collect($items)
                    ->filter(fn($jumlah) => $jumlah > 0)
                    ->map(fn($jumlah, $nama) => "$nama ($jumlah pcs)");

                    echo $filtered->isNotEmpty()
                    ? $filtered->join(', ')
                    : '-';
                    @endphp
                </div>
