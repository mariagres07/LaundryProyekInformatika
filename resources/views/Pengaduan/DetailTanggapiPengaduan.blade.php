...
@endif
            ...
            <form action="{{ route('pengaduan.kirim', $pengaduan->idPengaduan) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="tanggapan" id="tanggapan" class="form-control @error('tanggapan') is-invalid @enderror" 
                        rows="5" required placeholder="Tuliskan tanggapan untuk pengaduan ini...">{{ old('tanggapan', $pengaduan->tanggapanPengaduan ?? '') }}</textarea>
                    @error('tanggapan')
